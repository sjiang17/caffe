...
def ConvBNLayer_Share(net, from_layer, out_layer, use_bn, use_relu, num_output,
    kernel_size, pad, stride, dilation=1, use_scale=True, lr_mult=1,
    conv_prefix='', conv_postfix='', bn_prefix='', bn_postfix='_bn',
    scale_prefix='', scale_postfix='_scale', bias_prefix='', bias_postfix='_bias',
    head_type='', **bn_params):
  if use_bn:
    ...
    kwargs = {
        'param': [
            dict(name="mbox_{}_w".format(head_type), lr_mult=lr_mult, decay_mult=1),
            dict(name="mbox_{}_b".format(head_type), lr_mult=2 * lr_mult, decay_mult=0)],
        'weight_filler': dict(type='xavier'),
        'bias_filler': dict(type='constant', value=0)
        }
...
def ConvBNLayer50(net, from_layer, out_layer, use_bn, use_relu, num_output,
    kernel_size, pad, stride, dilation=1, use_scale=True, lr_mult=1,
    conv_prefix='', conv_postfix='', bn_prefix='', bn_postfix='_bn',
    scale_prefix='', scale_postfix='_scale', bias_prefix='', bias_postfix='_bias',
    **bn_params):
...
def ResNet50Body(net, from_layer, use_pool5=True, use_dilation_conv5=False, **bn_param):
    ...
    scale_prefix = 'scale_'
    scale_postfix = ''
    ConvBNLayer50(net, from_layer, 'conv1', use_bn=True, use_relu=True,
    ...
    ResBody(net, 'res4f', '5a', out2a=512, out2b=512, out2c=2048, stride=2, use_branch1=True, **bn_param)
...
def CreateMultiBoxHead_Share(net, data_layer="data", num_classes=[], from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, lr_mult=1,
        use_scale=True, min_sizes=[], max_sizes=[], prior_variance = [0.1],
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0,
    ...
    for i in range(0, num):
        ...
        if not share_location:
            num_loc_output *= num_classes
        ConvBNLayer_Share(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_loc_output, kernel_size=kernel_size, pad=pad, stride=1, head_type=head_type, **bn_param)
        permute_name = "{}_perm".format(name)
        net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
        flatten_name = "{}_flat".format(name)
        net[flatten_name] = L.Flatten(net[permute_name], axis=1)
        ...
        num_conf_output = num_priors_per_location * num_classes;
        ConvBNLayer_Share(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_conf_output, kernel_size=kernel_size, pad=pad, stride=1, head_type=head_type, **bn_param)
        permute_name = "{}_perm".format(name)
        net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
        flatten_name = "{}_flat".format(name)
        net[flatten_name] = L.Flatten(net[permute_name], axis=1)
