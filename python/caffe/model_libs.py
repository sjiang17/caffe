...
def CreateAnnotatedDataLayer(source, batch_size=32, backend=P.Data.LMDB,
        ...
        transform_param={}, batch_sampler=[{}], part_sampler_prob=0):
    if train:
        kwargs = {
    ...
    annotated_data_param = {
        ...
        'part_sampler_prob': part_sampler_prob,
        }
...
def CreateMultiBoxHead_Clean(net, data_layer="data", num_classes=[], from_layers=[],
        use_objectness=False, normalizations=[], use_batchnorm=True, lr_mult=1,
        use_scale=True, min_sizes=[], max_sizes=[], prior_variance = [0.1],
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0,
        conf_postfix='', loc_postfix='', clean_postfix='', **bn_param):
    assert num_classes, "must provide num_classes"
    assert num_classes > 0, "num_classes must be positive number"
    if normalizations:
        assert len(from_layers) == len(normalizations), "from_layers and normalizations should have same length"
    ...
    for i in range(0, num):
        ...
        # Create clean label prediction layer
        name = "{}_mbox_clean{}".format(from_layer, clean_postfix)
        num_clean_output = num_priors_per_location * 1;
        ConvBNLayer(net, from_layer, name, use_bn=use_batchnorm, use_relu=False, lr_mult=lr_mult,
            num_output=num_clean_output, kernel_size=kernel_size, pad=pad, stride=1, **bn_param)
        permute_name = "{}_perm".format(name)
        net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
        flatten_name = "{}_flat".format(name)
        net[flatten_name] = L.Flatten(net[permute_name], axis=1)
        clean_layers.append(net[flatten_name])
    ...
    if use_objectness:
        ...
        mbox_layers.append(net[name])
    name = "mbox_clean"
    net[name] = L.Concat(*clean_layers, axis=1)
    mbox_layers.append(net[name])
