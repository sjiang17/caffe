...
void DataTransformer<Dtype>::ExpandImage(const Datum& datum,
										...
										NormalizedBBox* expand_bbox,
										Datum* expand_datum,
										vector<float>* origin_coord) {
	//LOG(INFO) << "inside new expand image 2";
	// If datum is encoded, decode and crop the cv::image.
	if (datum.encoded()) {
#ifdef USE_OPENCV
		CHECK(!(param_.force_color() && param_.force_gray()))
		...
		// Expand the image.
		cv::Mat expand_img;
		ExpandImage(cv_img, expand_ratio, expand_bbox, &expand_img, origin_coord);
		// Save the image into datum.
		EncodeCVMatToDatum(expand_img, "jpg", expand_datum);
		expand_datum->set_label(datum.label());
		return;
	...
	// save the origin img coordinates w.r.t. expanded img
	(*origin_coord)[0] = w_off / width;
	(*origin_coord)[1] = h_off / height;
	(*origin_coord)[2] = static_cast<float>(datum_width) / width;
	(*origin_coord)[3] = static_cast<float>(datum_height) / height;
	//(*origin_coord)[4] = datum_width;
	//(*origin_coord)[5] = datum_height;
	//(*origin_coord)[6] = width;
	//(*origin_coord)[7] = height;
	//(*origin_coord)[8] = w_off;
	//(*origin_coord)[9] = h_off;
...
void DataTransformer<Dtype>::ExpandImage(const AnnotatedDatum& anno_datum,
	...
	if (prob > expand_prob) {
		...
		(*origin_coord)[2] = 1.f;
		(*origin_coord)[3] = 1.f;
		//(*origin_coord)[4] = anno_datum.datum().width();
		//(*origin_coord)[5] = anno_datum.datum().height();
		//(*origin_coord)[6] = anno_datum.datum().width();
		//(*origin_coord)[7] = anno_datum.datum().height();
		return;
	}
	const float max_expand_ratio = expand_param.max_expand_ratio();
	if (fabs(max_expand_ratio - 1.) < 1e-2) {
	...
	NormalizedBBox expand_bbox;
	ExpandImage(anno_datum.datum(), expand_ratio, &expand_bbox,
		expanded_anno_datum->mutable_datum(), origin_coord);
	expanded_anno_datum->set_type(anno_datum.type());
...
void DataTransformer<Dtype>::ExpandImage(const cv::Mat& img,
										...
										cv::Mat* expand_img,
										vector<float>* origin_coord) {
	//LOG(INFO) << "inside expand image 3 mat";
	//LOG(INFO) << "xmin: " << (*origin_coord)[0];
	//LOG(INFO) << "xmax: " << (*origin_coord)[1];
	//LOG(INFO) << "width: " << (*origin_coord)[2];
	//LOG(INFO) << "height: " << (*origin_coord)[3];
	...
	(*origin_coord)[0] = w_off / width;
	(*origin_coord)[1] = h_off / height;
	(*origin_coord)[2] = static_cast<float>(img_width) / width;
	(*origin_coord)[3] = static_cast<float>(img_height) / height;
	//(*origin_coord)[4] = static_cast<float>(img_width);
	//(*origin_coord)[5] = static_cast<float>(img_height);
	//(*origin_coord)[6] = width;
	//(*origin_coord)[7] = height;
	//(*origin_coord)[8] = w_off;
	//(*origin_coord)[9] = h_off;
